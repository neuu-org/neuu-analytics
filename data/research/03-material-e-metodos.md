...
Este trabalho adota uma abordagem experimental quantitativa, organizada em sete experimentos sequenciais, cada um isolando uma variável independente específica do pipeline de recuperação híbrida. O desenho progressivo permite atribuição causal: o Experimento 1 calibra parâmetros base, o Experimento 2 refina a curva de fusão léxico-semântica, o Experimento 3 avalia o impacto do modelo de embedding, o Experimento 4 testa fusão de embeddings multi-versão como experimento controlado negativo, o Experimento 5 introduz enriquecimento por conhecimento externo da literatura cristã clássica, o Experimento 6 avalia o impacto de embeddings large (3.072 dimensões) no corpus CCEL em comparação pareada controlada com embeddings small, e o Experimento 7 aplica o pipeline otimizado à descoberta automática de temas bíblicos, testando incrementalmente três técnicas da literatura recente de RAG (HyDE, Multi-Query e Cross-Encoder) sobre 50 clusters temáticos.
...
### Experimento 7 — Descoberta temática com melhorias incrementais

O Experimento 7 aplica o pipeline de recuperação híbrida otimizado nos Experimentos 1 a 6 a uma tarefa downstream: a descoberta automática de temas bíblicos a partir de clusters temáticos gerados por agrupamento semântico de propostas. O desenho é incremental em quatro estágios cumulativos sobre os mesmos 50 clusters temáticos selecionados dos 336 disponíveis: estágio 0 (baseline) utiliza o pipeline v2 com 6 camadas de evidência (hybrid search, CCEL bridge, cross-reference, definition agreement, anchor verses, centroid similarity); estágio 1 adiciona HyDE (Hypothetical Document Embeddings, Gao et al., 2022), que gera documentos hipotéticos via LLM para melhorar o recall semântico; estágio 2 adiciona Multi-Query com RAG-Fusion (Rackauckas, 2024), expandindo cada cluster em quatro queries alternativas (português, inglês, keywords, HyDE) com fusão via Reciprocal Rank Fusion; estágio 3 adiciona Cross-Encoder (Rosa et al., 2022), que reordena os candidatos processando cada par (descrição do tema, texto do versículo) conjuntamente. Todos os parâmetros do pipeline base são herdados do campeão do Experimento 6 (α=0,7, embedding large 3.072d, CCEL large w=0,5, deduplicação, MMR λ=0,3). As métricas de avaliação são específicas para theme discovery: promotion rate (percentual de clusters com score de evidência acima de 0,70), distribuição de grades (versículos classificados como Grade 3, 2 ou 1 conforme número de fontes independentes que os confirmam) e score médio de evidência.
